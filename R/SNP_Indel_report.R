...
SNP_Indel_report <- function(path, QD_SNPS = "< 2.0", FS_SNPS = "> 60.0", MQ_SNPS = "< 40.0", MQRankSum_SNPS = "< -12.5", ReadPosRankSum_SNPS = "< -8.0", QD_INDELS = "< 2.0", FS_INDELS = "> 200.0", ReadPosRankSum_INDELS = "< -20.0") {
  ...
  wb_SNP_Indel <- generate_XLSX(VCF_file)
...
bam_to_vcf <- function(sorted_BAM, QD_SNPS = "< 2.0", FS_SNPS = "> 60.0", MQ_SNPS = "< 40.0", MQRankSum_SNPS = "< -12.5", ReadPosRankSum_SNPS = "< -8.0", QD_INDELS = "< 2.0", FS_INDELS = "> 200.0", ReadPosRankSum_INDELS = "< -20.0") {
  ...
  # STEP 12
  system2("java", sprintf("-jar %s MergeVcfs -I %s%s_filtered_snps_tomerge.vcf -I %s%s_filtered_indels_tomerge.vcf -O %s%s_MitoR.vcf", PICARD, path_to_MitoR, patient_ID, path_to_MitoR, patient_ID, path_to_MitoR, patient_ID), stdout = TRUE, wait = TRUE)
  ...
  file.remove(sprintf("%s%s_filtered_MitoR.vcf.idx", path_to_MitoR, patient_ID))


  return(sprintf("%s%s_MitoR.vcf", path_to_MitoR, patient_ID))
}
