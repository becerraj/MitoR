...
pileupPlot <- function(BAM_file, plot_mutation = TRUE, range = 5) {
  ...
  for (i in (1:nrow(mutations_at_genes))) {
    ...
    system2("java", sprintf("-jar %s CollectAllelicCounts -R %s -O %s%s_region_allelic -I %s -L %s", GATK, referencia, path_to_MitoR, numPaciente, BAM_file, mutations_at_genes$Interval[i]), stdout = TRUE)

    # Takes the information given by GATK and creates a DF with it. Colomns: POS, REF Counts, ALT Counts, REF, ALT
    allelic_coverage <- readLines(sprintf("%s%s_region_allelic", path_to_MitoR, numPaciente))[c(-1,-2,-3, -4)] %>%
      strsplit("\t") %>% unlist()
  ...
  #xlsx_plot_mut_sheet <- xlsx_plot_design(plots_to_xlsx)
...
generateTXT_gene <- function(numPaciente) {
  # Load mutations
  mitor_files <- Sys.getenv('R_LIBS_USER')
  path <- as.character(readRDS(sprintf("%s/mitorDB/RDS_DB.rds", mitor_files))[[sprintf("%s", numPaciente)]][1])
  mutations <- readRDS(sprintf("%s/mitorDB/RDS_DB.rds", mitor_files))[[sprintf("%s", numPaciente)]]$Mutations
...
#' @param allelic_coverage DataFrame of colomns POS-REFCounts-ALTCounts-REF-ALT-TotalCounts
...
plot_gene_design <- function(allelic_coverage, mutations_at_genes) {
  ...
  ALTCounts <- allelic_coverage$`ALT Counts`
  par(xpd = TRUE)

  print(max(Counts))

  plot(Position, Counts, type = "l", col = "black", main = mutations_at_genes$gene, ylim = c(0, max(Counts)))
  lines(Position, ALTCounts, type = "o", col = "red") # Same values for X axis
...
#' @param data_mutation_plot DataFrame of colomns POS-REFCounts-ALTCounts-REF-ALT-TotalCounts for the mutated position and the <range> closests nucleotides
...
plot_mutation_design <- function(data_mutation_plot, alternative_base) {
  ...
  ALTCounts <- data_mutation_plot$`ALT Counts`
  ...
  lines(Position, ALTCounts, type = "b", pch = ALT_letters, col = "red")
