...
pileupPlot <- function(BAM_file, plot_mutation = TRUE, range = 5) {
  ...
  for (i in (1:nrow(mutations_at_genes))) {
    ...
    system2("java", sprintf("-jar %s CollectAllelicCounts -R %s -O %s%s_region_allelic -I %s -L %s", GATK, reference, path_to_MitoR, numPaciente, BAM_file, mutations_at_genes$Interval[i]), stdout = TRUE)
...
generateTXT_gene <- function(numPaciente) {
  ...
  path <- as.character(readRDS(sprintf("%s/RDS_DB.rds", mitor_db))[[sprintf("%s", numPaciente)]][1])
  mutations <- readRDS(sprintf("%s/RDS_DB.rds", mitor_db))[[sprintf("%s", numPaciente)]]$Mutations
