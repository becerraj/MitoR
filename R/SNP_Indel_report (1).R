...
SNP_Indel_report <- function(path, QD_SNPS = "< 2.0", FS_SNPS = "> 60.0", MQ_SNPS = "< 40.0", MQRankSum_SNPS = "< -12.5", ReadPosRankSum_SNPS = "< -8.0", QD_INDELS = "< 2.0", FS_INDELS = "> 200.0", ReadPosRankSum_INDELS = "< -20.0") {
  # Checks if path is a BAM file or the directory with the fasta/fastq files
  if (!substr(path, nchar(path) - 3, nchar(path)) == ".bam") {
  ...
  #wb_SNP_Indel_plot <- pileupPlot(BAM_file)
  plots_to_xlsx <-pileupPlot(BAM_file)
...
combine_workbooks <- function(workbooks, sheetNames) {
  # Creates a new workbook where all the other will be loaded
  wb_SNP_Indels <- openxlsx::createWorkbook()

  # Iterate through the list of workbooks
  for (i in (1:length(workbooks))) {
    openxlsx::addWorksheet(wb_SNP_Indels, sheetName = sheetNames[[i]])

    # Write the data in the resulting WB
    openxlsx::writeData(wb_SNP_Indels, sheet = sheetNames[[i]], x = workbooks[[i]])
  }
