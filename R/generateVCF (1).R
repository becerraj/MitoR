...
generate_VCF <- function(path, QD_SNPS = "< 2.0", FS_SNPS = "> 60.0", MQ_SNPS = "< 40.0", MQRankSum_SNPS = "< -12.5", ReadPosRankSum_SNPS = "< -8.0", QD_INDELS = "< 2.0", FS_INDELS = "> 200.0", ReadPosRankSum_INDELS = "< -20.0") {
  ...
  file.remove(sprintf("%s/MitoR/%s_dedup_reads.bam", path, titulo))
...
fasta_to_bam <- function(path_dir) {
  ...
  #Remove big files
  file.remove(sprintf("%s/MitoR/%s_R1R2.sam", path_file, id))
  file.remove(sprintf("%s/MitoR/%s_R1R2.bam", path_file, id))
