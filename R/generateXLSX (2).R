...
#' @title Check if there is a MTVAR value to complete
#' @description When doing the analysis of a patient you must be online. Otherwise MitoR won't be able to look for the mutations information at HMTVAR.
#' In case you analysed it offline, as soon as you call the MitoR library again, MitoR will check if there are any Offline values and will try to look for them online.
  ...
  if (!httr::http_error(httr::GET("https://www.google.com"))){
    ...
    if ("RDS_DB.rds" %in% list.files(mitor_files)){
      ...
      if (any(stringr::str_detect(RDS_DB[[1]]$Freq_HMTVAR , "Offline"))){
        Freq_MitoR <- RDS_DB[[1]]
        # Selects the colomns with the value "Offline"
        valuesOffline <- Freq_MitoR[Freq_MitoR$Freq_HMTVAR == "Offline", ]
        valuesToUpdate <- buscarHMTVAR(valuesOffline)
        # Adds the modified rows in the same place as the previous "Offline" rows were
        Freq_MitoR[valuesOffline, ] <- valuesToUpdate
        # Saves the updated DB as a RDS file
